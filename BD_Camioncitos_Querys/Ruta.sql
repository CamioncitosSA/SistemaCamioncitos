...
BEGIN
    ...
    IF NOT EXISTS(SELECT * FROM TB_RUTA WHERE IDRuta = @IDRuta)
    BEGIN
        SELECT 'ERROR: No se encontró la ruta con IDRuta: [' + CAST(@IDRuta AS VARCHAR) + ']';
        RETURN
    END

    -- Realiza las operaciones para generar el comprobante de entrega
    ...
    SELECT 'Se ha generado el comprobante de entrega para la ruta con IDRuta: [' + CAST(@IDRuta AS VARCHAR) + '].'
END;
